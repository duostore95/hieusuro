...
export async function registerRoutes(app: Express): Promise<Server> {
  // Note: Authentication routes are now handled by better-auth at /api/auth/*
  // No need to define login, logout, or change-password routes here
  ...
  app.post('/api/landing-views/:slug/view', async (req, res) => {
    try {
      ...
      const slug = req.params.slug.startsWith('/')
        ? req.params.slug
        : `/${req.params.slug}`;
      ...
      if (!slug.match(/^\/[a-zA-Z0-9\-_]+$/)) {
        return res
          .status(400)
          .json({ error: 'Invalid landing page slug format' });
      }

      const landingPageView = await storage.incrementLandingPageViewBySlug(
        slug
      );
  ...
  app.delete('/api/landing-views/reset', requireAuth, async (req, res) => {
    try {
      ...
      res.status(200).json({
        message:
          'Landing page views reset successfully to 0. Clean slate for accurate tracking.',
      });
    } catch (error) {
  ...
  app.get('/api/stats', requireAuth, async (req, res) => {
    try {
      const [posts, courses, testimonials, leads, landingPageViews] =
        await Promise.all([
  ...
  <url>
    ...
    <priority>0.7</priority>
      ...
      const activeCourses = courses.filter(
        (course) => course.status === 'active' && course.courseUrl
      );
